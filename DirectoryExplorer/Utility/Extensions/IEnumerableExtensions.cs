...
namespace DirectoryExplorer.Utility.Extensions
{
    static class IEnumerableExtensions
    {
        ...
        public static IEnumerable<(IEntity, IEntity)> AllInteractions(this IEnumerable<IEntity> enumerable) =>
            enumerable.SelectMany(a => enumerable.Where(b => a != b).Select(b => (a, b)));
        ...
        public static IEnumerable<(IEntity, IEntity)> IfDo<T1, T2>(this IEnumerable<(IEntity A, IEntity B)> enumerable, Action<T1, T2> action) where T1 : class where T2 : class =>
            enumerable.Select(e =>
            {
                if (e.A is T1 && e.B is T2) action(e.A as T1, e.B as T2);
                else if (e.B is T1 && e.A is T2) action(e.B as T1, e.A as T2);
                return e;
            });
        ...
        public static IEnumerable<(T1, T2)> Do<T1, T2>(this IEnumerable<(T1 A, T2 B)> enumerable, Action<T1, T2> action) where T1 : class where T2 : class =>
            enumerable.Select(e =>
            {
                action(e.A, e.B);
                return e;
